...
from models_pkg.nlp.nlp_pipeline import DialogueManager, response_generator

# Language label used to ask Claude to reply in the farmer's language
LANG_NAMES = {
    "en": "English", "hi": "Hindi (हिन्दी)", "te": "Telugu (తెలుగు)",
    "kn": "Kannada (ಕನ್ನಡ)", "ta": "Tamil (தமிழ்)", "mr": "Marathi (मराठी)",
}
...
class AgentReasoningEngine:
    ...
    async def process_query(self, message: str, language: str = "en") -> Dict:
        ...
        # Try Claude for a polished, multilingual response. Fall back to the
        # template ResponseGenerator so the chat never returns an empty string.
        ai_text = await self.generate_smart_response(message, result, language=language)
        if not ai_text:
            ai_text = self._template_response(result)
        result["ai_response"] = ai_text
        self.dialogue.reset()
        return result

    def _template_response(self, result: Dict) -> str:
        """Deterministic English response when Claude isn't available."""
        try:
            advisories = result.get("advisories", {})
            location = result.get("location", "")
            weather_summary = (result.get("weather") or {}).get("summary", {})
            slots = result.get("slots", {})
            if "crop" in advisories:
                d = advisories["crop"]
                return response_generator.generate_crop_response(
                    d.get("recommendations", []), location,
                    slots.get("TIMEFRAME", "7") + " days", weather_summary,
                )
            if "pesticide" in advisories:
                d = advisories["pesticide"]
                risks = d.get("risks", {})
                return response_generator.generate_pesticide_response(
                    risks.get("pests", []), risks.get("diseases", []),
                    d.get("recommendations", []), weather_summary,
                )
            if "fertilizer" in advisories:
                d = advisories["fertilizer"]
                return response_generator.generate_fertilizer_response(
                    d.get("options", []), d.get("crop", ""), d.get("growth_stage", ""),
                )
            if "irrigation" in advisories:
                return response_generator.generate_irrigation_response(advisories["irrigation"].get("analysis", {}))
            if "weather" in advisories:
                w = advisories["weather"]
                s = w.get("summary", {})
                return (f"Weather for {w.get('location', location)}: "
                        f"{s.get('temp_min','?')}–{s.get('temp_max','?')}°C, "
                        f"{s.get('humidity_avg','?')}% humidity, "
                        f"{s.get('rainfall_total','?')}mm rain expected over "
                        f"{w.get('forecast_days','?')} days "
                        f"({s.get('rainy_days',0)} rainy).")
            if "general" in advisories:
                return advisories["general"].get("message", "")
        except Exception as e:
            print(f"[template_response] {e}")
        return "Here's what I found from the latest weather and farm data."

    async def generate_smart_response(self, user_message: str, advisory_data: Dict, language: str = "en") -> str:
            ...
            async with httpx.AsyncClient(timeout=15) as client:
                resp = await client.post(
                    ...
                    json={
                        ...
                        "max_tokens": 300,
                        "system": (
                            "You are KrishiMitra (कृषि मित्र), an expert Indian agricultural advisor. "
                            "You speak to farmers in simple, practical language. Keep responses SHORT (3-5 sentences). "
                            "Be specific with numbers, dates, and actionable advice. Use the weather and advisory data "
                            "provided to give precise, location-specific guidance. Never say \"I don't know\" - always "
                            "give useful farming advice based on the data. Focus on what the farmer should DO this week. "
                            f"IMPORTANT: Respond in {LANG_NAMES.get(language, 'English')}. "
                            "If the user's language is not English, write the entire response in that language using its native script. "
                            "You may keep crop names, agronomy terms (urea, NPK, kharif, rabi) untranslated where natural."
                        ),
                        "messages": [
